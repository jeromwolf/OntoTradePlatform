          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="text-center">
                ...
                <div className={`text-2xl font-bold ${portfolioSummary.dailyChange >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {periods.map((period) => (
                <button
                  key={period.key}
                  onClick={() => setSelectedPeriod(period.key as '1M' | '3M' | '6M' | '1Y' | 'ALL')}
            ...
            <div className="bg-gray-700 rounded-lg p-6 h-64 flex items-center justify-center">
              <div className="text-center text-gray-400">
                ...
                <div className="text-sm mt-2">
                  {language === 'ko' ? '차트 라이브러리 통합 예정' : 'Chart library integration pending'}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-4 gap-2 text-sm font-medium text-gray-400 border-b border-gray-600 pb-2">
                    ...
                    <div className="text-center">{language === 'ko' ? '수량' : 'Qty'}</div>
                    <div className="text-center">{language === 'ko' ? '비중' : 'Weight'}</div>
                    <div className="text-center">{language === 'ko' ? '손익' : 'P&L'}</div>
                  </div>
                  
                  {positions.map((position) => (
                    <div key={position.id} className="grid grid-cols-4 gap-2 text-sm">
                      <div className="text-white font-medium">{position.symbol}</div>
                      <div className="text-center text-gray-300">{position.quantity}</div>
                      <div className="text-center text-gray-300">{position.weight}%</div>
              ...
              <div className="bg-gray-800 rounded-lg p-6 mt-6">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-2 text-sm font-medium text-gray-400 border-b border-gray-600 pb-2">
                    ...
                    <div className="text-center">{language === 'ko' ? '종목' : 'Symbol'}</div>
                    <div className="text-center">{language === 'ko' ? '유형' : 'Type'}</div>
                  </div>
                  
                  {transactions.slice(0, 5).map((transaction) => (
                    <div key={transaction.id} className="grid grid-cols-3 gap-2 text-sm">
                      <div className="text-gray-300">{transaction.date.slice(5)}</div>
                      <div className="text-center text-white font-medium">{transaction.symbol}</div>
                      <div className={`text-center font-medium ${
                ...
                <button className="w-full mt-4 bg-gray-700 hover:bg-gray-600 text-gray-300 font-medium py-2 px-4 rounded-lg transition-colors">
                  {language === 'ko' ? '전체 내역 보기' : 'View All Transactions'}
                </button>
              </div>
            </div>
