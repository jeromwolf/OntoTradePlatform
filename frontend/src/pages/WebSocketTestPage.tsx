...
import { StockSubscriptionManager, ConnectionStatus } from '../components/trading/StockRealTimeDisplay';
...
const WebSocketTestPage: React.FC = () => {
  ...
  const [subscribedSymbols, setSubscribedSymbols] = useState<string[]>(['AAPL', 'GOOGL', 'MSFT']);
  const { 
    connectionHistory, 
    reconnectAttempts, 
    getConnectionStats,
  ...
  const successRate = stats.total > 0 ? ((stats.connected / stats.total) * 100) : 0;

  useEffect(() => {
    if (isConnected && subscribedSymbols.length > 0) {
      console.log('WebSocket 연결됨, 기본 종목들 구독 시작:', subscribedSymbols);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">🔗 WebSocket 실시간 데이터 테스트</h1>
        ...
        <div className="bg-white p-6 rounded-lg border shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="p-4 bg-yellow-50 rounded-lg">
              <div className="text-2xl font-bold text-yellow-600">{reconnectAttempts}</div>
                ...
                {connectionHistory.slice(-10).reverse().map((entry, index) => (
                  <div key={index} className="flex items-center justify-between text-sm">
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      ...
                      entry.status === 'connecting' ? 'bg-yellow-100 text-yellow-800' :
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border">
        ...
        <div className="mt-4 p-3 bg-white rounded border-l-4 border-blue-400">
          <p className="text-blue-800 text-sm">
            💡 <strong>팁:</strong> 백엔드 서버가 실행되어 있어야 실시간 데이터를 받을 수 있습니다. 
            연결 상태가 "연결 중..."인 경우 백엔드 서버 상태를 확인해주세요.
          </p>
        </div>
      </div>
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">🔧 기술 정보</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <h3 className="font-medium text-gray-800 mb-2">WebSocket 엔드포인트</h3>
