...
export const useStockSubscription = (symbol: string | null) => {
  ...
  useEffect(() => {
    ...
    // 구독 확인 리스너 등록
    const subscriptionConfirmedCallback = (data: {
      symbol: string;
      status: string;
    }) => {
      if (data.symbol === symbol.toUpperCase()) {
...
export const useMultipleStockSubscriptions = (symbols: string[]) => {
  ...
  const [lastUpdates, setLastUpdates] = useState<Map<string, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    const newSymbols = symbols.filter(
      (symbol) => !previousSymbols.has(symbol.toUpperCase()),
    );

    // 제거된 심볼 구독 해제
    const removedSymbols = previousSymbolsRef.current.filter(
      (symbol) => !currentSymbols.has(symbol.toUpperCase()),
    );
    ...
    const stockUpdateCallback = (data: StockQuote) => {
      if (currentSymbols.has(data.symbol)) {
        ...
        setLastUpdates(
          (prev) => new Map(prev.set(data.symbol, new Date().toISOString())),
        );

        // 해당 심볼의 오류 제거
        setErrors((prev) =>
          prev.filter((error) => !error.includes(data.symbol)),
        );
      }
    };
...
export const useWebSocketMonitor = () => {
  ...
  const getConnectionStats = useCallback(() => {
    ...
    const disconnected = connectionHistory.filter(
      (h) => h.status === "disconnected",
    ).length;
    const connecting = connectionHistory.filter(
      (h) => h.status === "connecting",
    ).length;
