...
const DataQualityDashboard: React.FC<DataQualityDashboardProps> = ({ className = '' }) => {
  ...
  const getQualityStatus = (rate: number) => {
    ...
    if (rate >= 0.90) return { text: '양호', color: 'text-blue-600' };
    if (rate >= 0.80) return { text: '보통', color: 'text-yellow-600' };
    ...
    return (
      <div className={`bg-white rounded-lg shadow p-6 ${className}`}>
        <div className="flex items-center justify-center h-48">
          ...
          <span className="ml-2 text-gray-600">데이터 품질 정보 로딩 중...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`bg-white rounded-lg shadow p-6 ${className}`}>
        <div className="text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">데이터 품질 정보 로딩 실패</h3>
  ...
  if (!qualityReport) {
    return (
      <div className={`bg-white rounded-lg shadow p-6 ${className}`}>
        <div className="text-center text-gray-500">데이터 품질 정보가 없습니다.</div>
      </div>
    );
  }
    ...
    <div className={`bg-white rounded-lg shadow ${className}`}>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-500">이상치 발생률</p>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-500">모니터링 종목</p>
        ...
        {recent_anomalies.length > 0 && (
          <div className="mb-6">
            <h3 className="text-lg font-medium text-gray-900 mb-3">최근 이상치 알림</h3>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 text-sm opacity-75">{anomaly.type}</span>
                    </div>
        ...
        {/* 종목별 품질 메트릭 */}
        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-3">종목별 데이터 품질</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">종목</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">검증률</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">이상치율</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">처리시간</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">최종 업데이트</th>
                </tr>
              </thead>
