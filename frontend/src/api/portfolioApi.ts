...
export const getPortfolio = async (portfolioId: string): Promise<Portfolio> => {
  const response = await axios.get<Portfolio>(`${API_BASE_URL}/portfolios/${portfolioId}`);
  return response.data;
};

// 포트폴리오 생성
export const createPortfolio = async (data: PortfolioCreateData): Promise<Portfolio> => {
  const response = await axios.post<Portfolio>(`${API_BASE_URL}/portfolios/`, data);
  return response.data;
};
...
export const updatePortfolio = async (
  ...
  const response = await axios.put<Portfolio>(`${API_BASE_URL}/portfolios/${portfolioId}`, data);
  return response.data;
};

// 포트폴리오 삭제
export const deletePortfolio = async (portfolioId: string): Promise<{ id: string }> => {
  const response = await axios.delete<{ id: string }>(`${API_BASE_URL}/portfolios/${portfolioId}`);
  return response.data;
};

// 포트폴리오 보유 종목 조회
export const getPortfolioHoldings = async (portfolioId: string): Promise<Holding[]> => {
  const response = await axios.get<Holding[]>(`${API_BASE_URL}/portfolios/${portfolioId}/holdings`);
  return response.data;
};
...
// 포트폴리오 설정 조회
export const getPortfolioSettings = async (portfolioId: string): Promise<any> => {
  const response = await axios.get(`${API_BASE_URL}/portfolios/${portfolioId}/settings`);
  return response.data;
};
