...
interface PortfolioContextType extends PortfolioState {
  ...
  updatePortfolio: (portfolioId: string, data: Partial<PortfolioCreateData>) => Promise<void>;
  deletePortfolio: (portfolioId: string) => Promise<void>;
  
  // 보유 종목 관리
  loadHoldings: (portfolioId: string) => Promise<void>;
  addHolding: (portfolioId: string, data: HoldingCreateData) => Promise<void>;
  updateHolding: (portfolioId: string, holdingId: string, data: Partial<HoldingCreateData>) => Promise<void>;
  removeHolding: (portfolioId: string, holdingId: string) => Promise<void>;
  
  // 거래 내역 관리
  loadTransactions: (portfolioId: string) => Promise<void>;
  addTransaction: (portfolioId: string, data: TransactionCreateData) => Promise<void>;
  
  // 성과 데이터
  loadPerformance: (portfolioId: string, startDate?: string, endDate?: string) => Promise<void>;
...
export const PortfolioProvider: React.FC<PortfolioProviderProps> = ({ children }) => {
  ...
  const createPortfolio = async (data: PortfolioCreateData): Promise<Portfolio> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const updatePortfolio = async (portfolioId: string, data: Partial<PortfolioCreateData>) => {
    try {
  ...
  const updateHolding = async (portfolioId: string, holdingId: string, data: Partial<HoldingCreateData>) => {
    try {
      const holding = await portfolioApi.updateHolding(portfolioId, holdingId, data);
  ...
  const addTransaction = async (portfolioId: string, data: TransactionCreateData) => {
    try {
  ...
  // 성과 데이터 함수들
  const loadPerformance = async (portfolioId: string, startDate?: string, endDate?: string) => {
    try {
      const performance = await portfolioApi.getPerformance(portfolioId, startDate, endDate);
