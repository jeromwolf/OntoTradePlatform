...
async def health_check():
    ...
    try:
        ...
        capture_exception(
            e, {"endpoint": "GET /health/", "operation": "basic_health_check"}
        )
...
async def detailed_health_check():
    ...
    try:
        with logging_system.performance_monitor.measure_operation(
            "health_detailed_check"
        ) as monitor:
            log_info(
                "상세 헬스 체크 시작",
                category="health_check",
                context={"source": "detailed_health_endpoint"},
            )
        ...
        capture_exception(
            e,
            {"endpoint": "GET /health/detailed", "operation": "detailed_health_check"},
        )
    ...
    try:
        with logging_system.performance_monitor.measure_operation(
            ...
            log_info(
                "준비 상태 확인 완료",
                category="health_check",
                context={"status": response_data["status"], "checks": checks},
            )
        ...
        capture_exception(
            e, {"endpoint": "GET /health/readiness", "operation": "readiness_check"}
        )
    ...
    try:
        with logging_system.performance_monitor.measure_operation(
            ...
            log_info(
                "생존 상태 확인 완료",
                category="health_check",
                context={"status": response_data["status"]},
            )
        ...
        capture_exception(
            e, {"endpoint": "GET /health/liveness", "operation": "liveness_check"}
        )
