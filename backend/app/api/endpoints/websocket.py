...
async def broadcast_stock_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="데이터 전송 중 오류가 발생했습니다."
        )


@router.post("/broadcast/market-status")
async def broadcast_market_status(
    status: Dict[str, Any], current_user: Dict = Depends(get_current_user_optional)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="시장 상태 전송 중 오류가 발생했습니다."
        )
...
async def get_active_subscriptions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="구독 정보 조회 중 오류가 발생했습니다."
        )


@router.delete("/connections/{session_id}")
async def disconnect_client(
    session_id: str, current_user: Dict = Depends(get_current_user_optional)
):
    """
...
async def websocket_health_check():
    ...
    try:
        ...
        return JSONResponse(
            {"status": "healthy" if is_healthy else "unhealthy", "data": health_details}
        )
...
async def test_emit_data(
    event_name: str, data: Dict[str, Any], target_session: str = None
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="이벤트 전송 중 오류가 발생했습니다."
        )
