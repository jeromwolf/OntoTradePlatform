...
async def create_portfolio(
    ...
    try:
        ...
        log_error(f"portfolios_api create_portfolio error: user_id={current_user['id']}, error={str(e)}")
...
async def get_portfolios(
    ...
    try:
        ...
        log_error(f"portfolios_api get_portfolios error: user_id={current_user['id']}, error={str(e)}")
...
async def get_portfolio_detail(
    ...
    try:
        ...
        log_error(f"portfolios_api get_portfolio_detail error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_portfolio(
    ...
    try:
        ...
        log_error(f"portfolios_api update_portfolio error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_portfolio(
    ...
    try:
        ...
        log_error(f"portfolios_api delete_portfolio error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def execute_trade(
    ...
    try:
        ...
        log_info(f"거래 '{transaction_data.symbol}' '{transaction_data.transaction_type.value}' 성공")
        
        return JSONResponse(status_code=200, content=result)
    
    except ValueError as e:
        log_error(f"portfolios_api execute_trade error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        log_error(f"portfolios_api execute_trade error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_transaction_history(
    ...
    try:
        ...
        log_error(f"portfolios_api get_transaction_history error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        log_error(f"portfolios_api get_transaction_history error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_portfolio_settings(
    ...
    try:
        ...
        # 설정 조회
        settings_result = supabase.table("portfolio_settings").select("*").eq(
...
async def update_portfolio_settings(
    ...
    try:
        ...
        if settings_data.max_sector_exposure is not None:
            update_dict["max_sector_exposure"] = float(settings_data.max_sector_exposure)
        if settings_data.stop_loss_threshold is not None:
            update_dict["stop_loss_threshold"] = float(settings_data.stop_loss_threshold)
        if settings_data.take_profit_threshold is not None:
            update_dict["take_profit_threshold"] = float(settings_data.take_profit_threshold)
        if settings_data.rebalancing_frequency is not None:
            update_dict["rebalancing_frequency"] = settings_data.rebalancing_frequency.value
        ...
        # 설정 업데이트
        result = supabase.table("portfolio_settings").update(update_dict).eq(
            "portfolio_id", str(portfolio_id)
        ).execute()
        ...
        log_error(f"portfolios_api update_portfolio_settings error: user_id={current_user['id']}, error={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))