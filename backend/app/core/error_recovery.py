...
class ErrorRecoverySystem:
    ...
    async def _check_websocket_health(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                ...
                response_time_ms=0,
                timestamp=datetime.now(),
                details={"active_connections": 12, "message_queue_size": 0},
            )
        except Exception as e:
            return HealthCheck(
                service_name="websocket",
    ...
    def _update_service_health(
        self, service_name: str, check_result: HealthCheck
    ) -> None:
        """서비스 헬스 상태 업데이트"""
        ...
        if service.current_status != check_result.status:
            log_info(
                ...
                context={
                    ...
                    "old_status": service.current_status.value,
                    "new_status": check_result.status.value,
                },
            )
        ...
        # 성공률 계산
        healthy_checks = len(
            [c for c in service.recent_checks if c.status == ServiceStatus.HEALTHY]
        )
        service.success_rate = (
            (healthy_checks / len(service.recent_checks)) * 100
        ...
        # 평균 응답 시간 계산
        if service.recent_checks:
            service.average_response_time = sum(
                c.response_time_ms for c in service.recent_checks
            ) / len(service.recent_checks)
    ...
    async def _trigger_recovery(self, service_name: str) -> None:
        ...
        if success:
            ...
            if strategy.fallback_strategy:
                await self._execute_fallback_strategy(
                    service_name, strategy.fallback_strategy
                )

    async def _execute_recovery_strategy(
        self, service_name: str, strategy: RecoveryStrategy
    ) -> bool:
        """복구 전략 실행"""
    ...
    async def _execute_recovery_action(
        self, service_name: str, action: RecoveryAction
    ) -> None:
        """개별 복구 액션 실행"""
    ...
    async def _execute_fallback_strategy(
        self, service_name: str, fallback_name: str
    ) -> None:
        """폴백 전략 실행"""
        log_warning(
