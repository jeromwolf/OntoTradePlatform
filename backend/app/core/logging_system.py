...
class LoggingSystem:
    ...
    def _update_error_metrics(
        self, category: ErrorCategory, severity: Optional[ErrorSeverity]
    ) -> None:
        """오류 메트릭 업데이트"""
    ...
    def _check_alert_thresholds(
        self, category: Optional[ErrorCategory], severity: Optional[ErrorSeverity]
    ) -> None:
        """알림 임계값 확인"""
    ...
    def _get_top_errors(
        self, logs: List[LogEntry], limit: int = 5
    ) -> List[Dict[str, Any]]:
        """상위 오류 목록 조회"""
        error_counts = {}
        ...
        return sorted(error_counts.values(), key=lambda x: x["count"], reverse=True)[
            :limit
        ]
    ...
    async def save_logs_to_file(self, filename: Optional[str] = None) -> str:
        ...
        async with aiofiles.open(filename, "w", encoding="utf-8") as f:
            await f.write(
                json.dumps(log_data, default=str, ensure_ascii=False, indent=2)
            )
...
def log_error(
    ...
    **kwargs,
    ...
    """오류 로그"""
    logging_system.log(
        LogLevel.ERROR, message, category=category, severity=severity, **kwargs
    )
...
def log_critical(
    ...
    **kwargs,
    ...
    """중요 오류 로그"""
    logging_system.log(
        LogLevel.CRITICAL, message, category=category, severity=severity, **kwargs
    )
