...
from app.core.logging_system import (
    log_info, log_warning, log_error, log_critical, log_websocket_event,
...
class WebSocketManager:
    ...
    def _register_events(self):
        ...
        async def connect(sid, environ):
            ...
            try:
                ...
                await self.sio.emit('connected', {'status': 'success', 'session_id': sid}, room=sid)
        ...
        async def subscribe(sid, data):
            ...
            try:
                ...
                log_websocket_event(
                    "symbol_subscribed",
                    context={"session_id": sid, "symbol": symbol}
                )
                ...
                log_error(
                    ...
                    category=ErrorCategory.WEBSOCKET_ERROR,
                    severity=ErrorSeverity.MEDIUM,
                    context={"session_id": sid, "symbol": data.get('symbol'), "error": str(e)},
                    logger_name="websocket"
                )
                
                await self.sio.emit('error', {
                    'message': f'구독 실패: {str(e)}',
        ...
        async def get_subscriptions(sid, data):
            ...
            try:
                ...
                await self.sio.emit('subscriptions', {
                    'symbols': symbols,
                    'count': len(symbols)
                }, room=sid)
    ...
    async def _subscribe_symbol(self, sid: str, symbol: str):
        ...
        try:
            async with logging_system.performance_monitor("stock_data_fetch"):
                ...
                await self.sio.emit('stock_data', {
                    'symbol': symbol,
            ...
            try:
                ...
                for symbol in list(self.subscriptions.keys()):
                    if symbol in self.subscriptions and self.subscriptions[symbol]:
                        try:
                            async with logging_system.performance_monitor(f"auto_update_{symbol}"):
    ...
    async def _send_stock_update(self, symbol: str):
        ...
        try:
            ...
            async with circuit_breaker("stock_data") as protected_call:
                stock_data = await protected_call(self.stock_simulator.get_real_time_data, symbol)
    ...
    def get_stats(self) -> dict:
        """WebSocket 통계 정보"""
        total_subscriptions = sum(len(sessions) for sessions in self.subscriptions.values())
