...
class WebSocketManager:
    ...
    async def unsubscribe_from_stock(self, sid: str, symbol: str) -> None:
        ...
        await self.sio.emit(
            ...
            {"symbol": symbol, "status": "unsubscribed"},
            room=sid,
        )
    ...
    async def broadcast_stock_data(self, symbol: str, data: Dict[str, Any]) -> None:
        ...
        if symbol in self.stock_subscribers:
            ...
            if subscribers:
                ...
                logger.debug(
                    f"{symbol} 데이터를 {len(subscribers)}명의 구독자에게 전송"
                )
        ...
        if self.active_connections:
            try:
                await self.sio.emit(
                    ...
                    {"status": status, "timestamp": asyncio.get_event_loop().time()},
                )
                logger.debug(
                    f"시장 상태를 {len(self.active_connections)}명의 클라이언트에게 전송"
                )
            except Exception as e:
...
async def subscribe_stock(sid, data):
    ...
    try:
        ...
        await sio.emit(
            "error", {"message": "구독 처리 중 오류가 발생했습니다."}, room=sid
        )
...
async def unsubscribe_stock(sid, data):
    ...
    try:
        ...
        await sio.emit(
            "error", {"message": "구독 해제 처리 중 오류가 발생했습니다."}, room=sid
        )
...
async def get_subscriptions(sid):
    ...
    try:
        if sid in ws_manager.active_connections:
            ...
            await sio.emit(
                "current_subscriptions", {"subscriptions": subscriptions}, room=sid
            )
        else:
            await sio.emit(
                "error", {"message": "연결 정보를 찾을 수 없습니다."}, room=sid
            )
    except Exception as e:
        logger.error(f"구독 목록 조회 오류: {e}")
        await sio.emit(
            "error", {"message": "구독 목록 조회 중 오류가 발생했습니다."}, room=sid
        )
...
async def ping(sid, data):
    ...
    await sio.emit(
        "pong",
        {"message": "pong", "timestamp": asyncio.get_event_loop().time()},
        room=sid,
    )
