...
class StockDataValidator:
    ...
    def validate_price(self, price: Union[str, float, int], field_name: str = "price") -> ValidationResult:
        """가격 데이터 검증"""
        errors = []
        warnings = []
        ...
        try:
            ...
            if normalized_price < self.MIN_PRICE:
                errors.append(f"{field_name}이 최소값({self.MIN_PRICE})보다 작습니다: {normalized_price}")
            elif normalized_price > self.MAX_PRICE:
                errors.append(f"{field_name}이 최대값({self.MAX_PRICE})보다 큽니다: {normalized_price}")
        ...
        return ValidationResult(
            ...
            errors=errors,
            warnings=warnings,
            normalized_data={field_name: float(normalized_price) if normalized_price else None}
        )
    ...
    def validate_percentage(self, percentage: Union[str, float], field_name: str = "change_percent") -> ValidationResult:
        """백분율 데이터 검증"""
        errors = []
        warnings = []
        ...
        try:
            ...
            if normalized_percent < self.MIN_CHANGE_PERCENT:
                errors.append(f"{field_name}이 허용 범위를 벗어남: {normalized_percent}%")
    ...
    def validate_timestamp(self, timestamp: Union[str, datetime], field_name: str = "timestamp") -> ValidationResult:
        """타임스탬프 검증"""
        errors = []
        warnings = []
        ...
        try:
            ...
            # 너무 오래된 날짜 검증
            if normalized_timestamp and normalized_timestamp < now - timedelta(days=3650):  # 10년
        ...
        return ValidationResult(
            ...
            errors=errors,
            warnings=warnings,
            normalized_data={field_name: normalized_timestamp.isoformat() if normalized_timestamp else None}
        )
    
    def detect_price_anomaly(self, current_price: float, previous_prices: List[float]) -> bool:
        """가격 이상치 탐지"""
    ...
    def validate_stock_quote(self, data: Dict[str, Any], source: DataSource = DataSource.UNKNOWN) -> ValidationResult:
        """전체 주식 시세 데이터 검증"""
        all_errors = []
        all_warnings = []
        normalized_data = {}
        ...
        validations = [
            ...
            self.validate_price(data.get("open"), "open") if data.get("open") is not None else ValidationResult(True, [], []),
            self.validate_price(data.get("high"), "high") if data.get("high") is not None else ValidationResult(True, [], []),
            self.validate_price(data.get("low"), "low") if data.get("low") is not None else ValidationResult(True, [], []),
            self.validate_price(data.get("previous_close"), "previous_close") if data.get("previous_close") is not None else ValidationResult(True, [], []),
            self.validate_volume(data.get("volume")) if data.get("volume") is not None else ValidationResult(True, [], []),
            self.validate_percentage(data.get("change_percent")) if data.get("change_percent") is not None else ValidationResult(True, [], []),
        ]
        
        # 타임스탬프 검증
        for timestamp_field in ["timestamp", "latest_trading_day"]:
            if data.get(timestamp_field) is not None:
                validations.append(self.validate_timestamp(data.get(timestamp_field), timestamp_field))
        ...
        # 경고가 있으면 로깅
        if all_warnings:
            logger.warning(f"데이터 검증 경고 ({data.get('symbol', 'Unknown')}): {', '.join(all_warnings)}")
