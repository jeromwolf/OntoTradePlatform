...
class TradingService:
    ...
    async def _validate_portfolio(self, portfolio_id: UUID, user_id: UUID) -> Optional[Dict]:
        """포트폴리오 유효성 검증"""
    ...
    async def _validate_trade(
        ...
        """거래 유효성 검증"""
        total_cost = transaction_data.quantity * transaction_data.price + transaction_data.fees
    ...
    async def _execute_buy_order(
        ...
        tx_result = self.supabase.table("portfolio_transactions").insert(
            transaction_dict
        ).execute()
    ...
    async def _execute_sell_order(
        ...
        """매도 주문 실행"""
        total_proceeds = (transaction_data.quantity * transaction_data.price) - transaction_data.fees
    ...
    async def _get_holding(self, portfolio_id: UUID, symbol: str) -> Optional[Dict]:
        """보유 종목 조회"""
        result = self.supabase.table("portfolio_holdings").select("*").eq(
    ...
    async def _update_holding_sell(self, transaction_data: TransactionCreate):
        ...
        if existing_holding:
            ...
            realized_pnl = (transaction_data.price - avg_cost) * transaction_data.quantity
            ...
            if new_quantity == 0:
                ...
                self.supabase.table("portfolio_holdings").update({
                    "quantity": new_quantity,
                    "realized_pnl": float(
                        Decimal(str(existing_holding["realized_pnl"])) + realized_pnl
                    ),
                    "last_updated": datetime.utcnow().isoformat()
                }).eq("id", existing_holding["id"]).execute()
    ...
    async def get_transaction_history(
        ...
        try:
            ...
            result = self.supabase.table("portfolio_transactions").select("*").eq(
                "portfolio_id", str(portfolio_id)
