...
class SupabaseService:
    ...
    async def get_simulation_session(self, user_id: str) -> Optional[Dict]:
        ...
        try:
            result = (
                self.supabase.table("simulation_sessions")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def create_simulation_session(
        self, user_id: str, cash: float = 100000000
    ) -> Dict:
        """새 시뮬레이션 세션 생성"""
        try:
            session_data = {
    ...
    async def update_simulation_session(
        ...
        try:
            ...
            result = (
                await self.supabase.table("simulation_sessions")
                .update(update_data)
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def get_simulation_holdings(self, user_id: str) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.table("simulation_holdings")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def upsert_holding(
        self, user_id: str, symbol: str, quantity: int, avg_price: float
    ) -> Dict:
        """보유종목 추가/업데이트"""
        try:
            if quantity == 0:
                # 수량이 0이면 삭제
                result = (
                    await self.supabase.table("simulation_holdings")
                    .delete()
    ...
    async def add_transaction(
        self, user_id: str, symbol: str, type: str, quantity: int, price: float
    ) -> bool:
        """거래내역 추가"""
        try:
            transaction_data = {
            ...
            result = (
                self.supabase.table("simulation_transactions")
    ...
    async def get_simulation_leaderboard(self, limit: int = 20) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.table("simulation_sessions")
                .select("*")
                .order("total_pnl_percent", desc=True)
                .limit(limit)
                .execute()
            )
    ...
    async def get_watchlist(self, user_id: str) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.table("user_watchlists")
                .select("*")
                .eq("user_id", user_id)
                .order("created_at", desc=True)
                .execute()
    ...
    async def add_to_watchlist(
        self, user_id: str, symbol: str, name: str, **kwargs
    ) -> bool:
        """관심종목 추가"""
        try:
            watchlist_data = {
            ...
            result = (
                self.supabase.table("user_watchlists").insert(watchlist_data).execute()
            )
            return len(result.data) > 0
