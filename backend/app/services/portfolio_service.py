...
from app.core.logging_system import log_api_call, log_error, log_info
...
class PortfolioService:
    ...
    async def create_portfolio(
        ...
        try:
            log_info(
                f"새 포트폴리오 생성 시작: {portfolio_data.name}", category="portfolio"
            )
            ...
            portfolio_dict = {
                ...
                "target_return": (
                    float(portfolio_data.target_return)
                    if portfolio_data.target_return
                    else None
                ),
                "is_active": True,
                "metadata": {},
            ...
            log_info(
                f"포트폴리오 생성 완료: {portfolio_data_result['name']}",
                category="portfolio",
            )
    ...
    async def get_portfolios(self, user_id: UUID) -> List[Portfolio]:
        ...
        try:
            ...
            log_info(
                f"포트폴리오 목록 조회 완료: {len(portfolios)}개", category="portfolio"
            )
    ...
    async def get_portfolio_detail(
        ...
        try:
            ...
            # 설정 조회
            settings_result = (
                self.supabase.table("portfolio_settings")
                .select("*")
                .eq("portfolio_id", str(portfolio_id))
                .single()
                .execute()
            ...
            settings = (
                Settings(**settings_result.data) if settings_result.data else None
            )
            ...
            log_info(
                f"포트폴리오 상세 조회 완료: {portfolio.name}", category="portfolio"
            )
            return detail
    ...
    async def add_holding(self, holding_data: HoldingCreate) -> Holding:
        ...
        try:
            holding_dict = {
                ...
                "market_value": float(
                    holding_data.average_cost * holding_data.quantity
                ),
                "unrealized_pnl": 0.0,
                "realized_pnl": 0.0,
    ...
    async def record_transaction(
        ...
        try:
            ...
            log_info(
                f"거래 내역 기록 완료: {transaction.symbol} {transaction.transaction_type}",
                category="portfolio",
            )
    ...
    async def get_portfolio_performance(
        ...
        try:
            ...
            log_info(
                f"성과 데이터 조회 완료: {len(performances)}개", category="portfolio"
            )
    ...
    async def _update_holdings_after_transaction(
        self, transaction_data: TransactionCreate
    ):
        """거래 후 보유 종목 업데이트"""
        try:
            ...
            existing_result = (
                self.supabase.table("portfolio_holdings")
                .select("*")
                .eq("portfolio_id", str(transaction_data.portfolio_id))
                .eq("symbol", transaction_data.symbol)
                .execute()
            )
            ...
            if existing_result.data:
                ...
                if transaction_data.transaction_type == TransactionType.BUY:
                    new_quantity = (
                        existing_holding["quantity"] + transaction_data.quantity
                    )
                    total_cost = (
                        existing_holding["average_cost"] * existing_holding["quantity"]
                    ) + (transaction_data.price * transaction_data.quantity)
