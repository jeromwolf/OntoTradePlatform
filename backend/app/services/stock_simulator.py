...
from app.services.data_normalizer import data_normalizer, DataSource
...
class StockDataSimulator:
    ...
    def initialize_stock_data(self):
        ...
        for symbol, info in self.stock_symbols.items():
            ...
            validation_result = data_normalizer.normalize_and_validate(symbol, initial_data, DataSource.MOCK)
            
            if validation_result.is_valid:
                self.stock_data[symbol] = validation_result.normalized_data
                logger.debug(f"{symbol} 초기 데이터 정규화 성공 (품질 점수: {validation_result.normalized_data.get('data_quality', 'N/A')})")
                
                # 경고가 있는 경우 로깅
                if validation_result.warnings:
                    logger.warning(f"{symbol} 초기 데이터 경고: {', '.join(validation_result.warnings)}")
            else:
                logger.error(f"{symbol} 초기 데이터 정규화 실패: {validation_result.errors}")
                # 검증 실패 시 기본 데이터 사용
    ...
    def generate_price_update(self, symbol: str) -> Dict:
        ...
        # 데이터 정규화 및 검증 (향상된 버전 사용)
        validation_result = data_normalizer.normalize_and_validate(symbol, updated_data, DataSource.MOCK)
        ...
        if validation_result.is_valid:
            ...
            logger.log(log_level, f"{symbol} 데이터 업데이트: ${new_price} (품질: {validation_result.normalized_data.get('data_quality', 'N/A')})")
            
            # 이상치 경고 로깅
            if validation_result.warnings:
                logger.warning(f"{symbol} 이상치 탐지: {', '.join(validation_result.warnings)}")
            
            return validation_result.normalized_data.copy()
        else:
            # 검증 실패 시 경고 로그 및 이전 데이터 유지
            logger.error(f"{symbol} 데이터 업데이트 정규화 실패: {validation_result.errors}")
            # 타임스탬프만 업데이트하여 반환
                    ...
                    for symbol in active_symbols:
                        if symbol in self.stock_data:
                            ...
                            if updated_data:
                                ...
                                await websocket_manager.broadcast_stock_data(symbol, updated_data)
                                logger.debug(f"{symbol} 데이터 업데이트: ${updated_data['price']}")
    ...
    def get_data_quality_report(self) -> Dict:
        ...
        avg_validation_rate = sum(m.validation_rate for m in quality_metrics.values()) / total_symbols if total_symbols > 0 else 0
        avg_anomaly_rate = sum(m.anomaly_rate for m in quality_metrics.values()) / total_symbols if total_symbols > 0 else 0
        avg_processing_time = sum(m.processing_time for m in quality_metrics.values()) / total_symbols if total_symbols > 0 else 0
