...
class StockSearchService:
    ...
    async def search_stocks(
        self, query: str, market: str = "all", limit: int = 50
            ...
            async with self.session.get(url, params=params) as response:
                ...
                for match in data["bestMatches"]:
                    result = StockSearchResult(
                        ...
                        market=self._parse_market_from_alpha(
                            match.get("4. region", "")
                        ),
    ...
    async def _search_yahoo_finance(
        ...
        try:
            ...
            results = await loop.run_in_executor(
                None, self._yahoo_search_sync, query, market
            )
            return results
        except Exception as e:
    ...
    def _generate_symbol_patterns(self, query: str, market: str) -> List[str]:
        ...
        if market in ["all", "us"]:
            # 미국 주식 패턴들
            patterns.extend([query.upper(), f"{query.upper()}.US"])
    ...
    def _deduplicate_results(
        self, results: List[StockSearchResult]
    ) -> List[StockSearchResult]:
        """심볼 기준으로 중복 제거"""
        seen_symbols = set()
        unique_results = []
    ...
    def _sort_by_relevance(
        self, results: List[StockSearchResult], query: str
    ) -> List[StockSearchResult]:
        """검색어와의 관련성으로 정렬"""
