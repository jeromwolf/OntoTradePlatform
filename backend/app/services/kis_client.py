...
class KISClient:
    ...
    async def get_access_token(self, force_refresh: bool = False) -> str:
        ...
        if (
            ...
            wait_seconds = 60
            logger.warning(
                f"토큰이 만료되어 {wait_seconds}초 대기 후 갱신을 시도합니다."
            )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            logger.error(
                                f"KIS API 토큰 발급 실패 (시도 {attempt + 1}/{max_retries}): {error_text}"
                            )

            except Exception as e:
                logger.error(
                    f"KIS API 연결 오류 (시도 {attempt + 1}/{max_retries}): {str(e)}",
                    exc_info=True,
                )
            ...
            async with self.session.request(
                ...
                if response.status == 200:
                    ...
                    return await self._make_request(
                        method, endpoint, headers, params, data, False
                    )
                else:
                    error_text = await response.text()
                    logger.error(f"KIS API 요청 실패 ({response.status}): {error_text}")
                    raise Exception(
                        f"KIS API 요청 실패 ({response.status}): {error_text}"
                    )
    ...
    # 해외주식 현재가 조회
    async def get_overseas_stock_quote(
        self, symbol: str, exchange: str = "NASD"
    ) -> Dict[str, Any]:
        """해외주식 현재가 조회"""
    ...
    async def get_korean_portfolio(self) -> Dict[str, Any]:
        ...
        headers = {"tr_id": "TTTC8434R", "custtype": "P"}  # 주식잔고조회  # 개인고객

        return await self._make_request("GET", endpoint, headers=headers, params=params)

    # 범용 주문 메서드 (기존 메서드와의 호환성을 위해 유지)
    async def place_order(
        self, order_data: Dict[str, Any], is_overseas: bool = False
    ) -> Dict[str, Any]:
