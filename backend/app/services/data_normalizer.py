...
from app.services.data_validator import validator, ValidationResult, DataSource
...
class DataNormalizer:
    ...
    def normalize_and_validate(self, symbol: str, data: Dict[str, Any], 
                              source: DataSource = DataSource.UNKNOWN) -> ValidationResult:
        """데이터 정규화 및 검증을 수행합니다."""
        ...
        try:
            ...
            # 5. 이상치가 발견된 경우 경고 추가
            if anomalies:
                validation_result.warnings.extend([alert.message for alert in anomalies])
    ...
    def _detect_anomalies(self, symbol: str, data: Dict[str, Any]) -> List[AnomalyAlert]:
        """이상치 탐지를 수행합니다."""
        anomalies = []
    ...
    def _check_data_integrity(self, symbol: str, data: Dict[str, Any]) -> Optional[AnomalyAlert]:
        """데이터 무결성을 검사합니다."""
        current_time = datetime.now()
    ...
    def _enhance_normalization(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for percent_field in ['change_percent']:
            if percent_field in enhanced_data and enhanced_data[percent_field] is not None:
                enhanced_data[percent_field] = round(float(enhanced_data[percent_field]), 4)
        ...
        enhanced_data['data_quality'] = self._calculate_data_quality_score(enhanced_data)
    ...
    def _update_quality_metrics(self, symbol: str, success: bool, start_time: datetime, 
        ...
        if symbol not in self.quality_metrics:
            ...
            alpha = 0.01  # 가중치
            metrics.validation_rate = (1 - alpha) * metrics.validation_rate + alpha * (1.0 if success else 0.0)
            metrics.anomaly_rate = (1 - alpha) * metrics.anomaly_rate + alpha * (1.0 if has_anomaly else 0.0)
            metrics.processing_time = (1 - alpha) * metrics.processing_time + alpha * processing_time
    ...
    def get_quality_metrics(self, symbol: Optional[str] = None) -> Dict[str, DataQualityMetrics]:
        """품질 메트릭을 조회합니다."""
        if symbol:
            return {symbol: self.quality_metrics.get(symbol)} if symbol in self.quality_metrics else {}
