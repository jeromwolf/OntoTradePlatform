...
def apply_schema():
    ...
    try:
        ...
        for i, statement in enumerate(sql_statements):
            ...
            try:
                # RPC 호출로 SQL 실행
                result = supabase_client.client.rpc('execute_sql', {'sql_query': statement}).execute()
                success_count += 1
...
def verify_tables():
    ...
    try:
        ...
        if len(existing_tables) == len(expected_tables):
            print(f"🎉 모든 포트폴리오 테이블 생성 완료! ({len(existing_tables)}/{len(expected_tables)})")
            return True
        else:
            print(f"⚠️ 일부 테이블만 생성됨: {len(existing_tables)}/{len(expected_tables)}")
...
if __name__ == "__main__":
    ...
    if apply_schema():
        ...
        print("📝 실행 명령어: uvicorn app.main:socket_app --host 0.0.0.0 --port 8000 --reload")
    else:
