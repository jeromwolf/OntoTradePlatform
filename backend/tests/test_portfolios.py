...
from app.models.portfolio import PortfolioCreate, SettingsUpdate, TransactionCreate
...
def test_create_portfolio(
    client: TestClient, test_user_token: str, portfolio_data: Dict[str, Any]
):
    """포트폴리오 생성 테스트"""
...
def test_get_portfolio_detail(
    client: TestClient, test_user_token: str, portfolio_id: str
):
    """포트폴리오 상세 조회 테스트"""
    response = client.get(
...
def test_update_portfolio_settings(
    client: TestClient, test_user_token: str, portfolio_id: str
):
    """포트폴리오 설정 업데이트 테스트"""
    update_data = {
