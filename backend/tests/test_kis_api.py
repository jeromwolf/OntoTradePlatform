...
class KISClient:
    ...
    async def get_access_token(self, force_refresh=False):
        ...
        if (
            ...
            print(
                f"⚠️ 토큰이 만료되었습니다. {wait_seconds}초 후에 갱신을 시도합니다..."
            )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            print(
                                f"❌ 토큰 발급 실패 (시도: {attempt + 1}): {response.status}"
                            )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, headers=headers, params=params) as response:
                    ...
                    async with session.get(
                        daily_url, headers=headers, params=daily_params
                    ) as daily_response:
                    ...
                    # 3. 호가 정보 조회
                    quote_url = (
                        f"{self.BASE_URL}/uapi/overseas-price/v1/quotations/ccol"
                    )
                    quote_params = {"AUTH": "", "EXCD": exchange, "SYMB": symbol}

                    async with session.get(
                        quote_url, headers=headers, params=quote_params
                    ) as quote_response:
    ...
    async def order_overseas_stock(
        ...
        headers = {
            ...
            "appsecret": self.app_secret,
            "tr_id": (
                "JTTT1002U" if side.upper() == "BUY" else "JTTT1006U"
            ),  # 매수/매도에 따라 다른 TR_ID
            "custtype": "P",
            "content-type": "application/json; charset=utf-8",
        }
    ...
    async def get_overseas_order_detail(self, order_no: str):
        ...
        params = {
            ...
            "ORD_STRT_DT": (datetime.now() - timedelta(days=7)).strftime(
                "%Y%m%d"
            ),  # 조회시작일 (최근 1주일)
    ...
    async def get_stock_price(
        self, symbol: str, is_overseas: bool = False, exchange: str = "NAS"
    ):
        """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, params=params) as response:
                    if response.status == 200:
                        ...
                        error = await response.text()
                        print(
                            f"❌ {'해외' if is_overseas else '국내'}주식 조회 실패: {response.status}"
                        )
                        print(f"에러 메시지: {error}")
                        return None
        except Exception as e:
...
async def test_overseas_trading():
    ...
    if balance and "output1" in balance and balance["output1"]:
        ...
        print(
            f"   평가손익: ${float(account.get('EVLU_PFLS_AMT', 0)):,.2f} ({float(account.get('EVLU_PFLS_RT', 0))}%)"
        )
        ...
        if "output2" in balance and balance["output2"]:
            ...
            for stock in balance["output2"]:
                ...
                print(
                    f"   현재가: ${float(stock.get('OVRS_STCK_AVAL_STL_AMT', 0)):,.2f}"
                )
                print(
                    f"   평가손익: ${float(stock.get('EVLU_PFLS_AMT', 0)):,.2f} ({float(stock.get('EVLU_PFLS_RT', 0))}%)"
                )
    else:
    ...
    if aapl_info and "output" in aapl_info and aapl_info["output"]:
        ...
        if buy_order and "output" in buy_order:
            print(
                f"✅ 주문 접수 완료: {json.dumps(buy_order['output'], indent=2, ensure_ascii=False)}"
            )
        ...
        if sell_order and "output" in sell_order:
            print(
                f"✅ 주문 접수 완료: {json.dumps(sell_order['output'], indent=2, ensure_ascii=False)}"
            )
        else:
            print("❌ 주문 실패")
            if sell_order:
...
async def assert_response_success(response: Dict[str, Any], expected_keys: list = None):
    ...
    assert (
        response["rt_cd"] == "0"
    ), f"API 요청 실패: {response.get('msg1', '알 수 없는 오류')}"
...
if __name__ == "__main__":
    ...
    if missing_vars:
        ...
        print(
            "프로젝트 루트 디렉토리에 .env 파일을 생성하고 해당 변수들을 설정해주세요."
        )
        exit(1)
